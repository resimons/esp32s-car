...
void showDegrees(float f, uint8_t brightness) {
    float round = 0.5;
    if (f < 0) {
        round *= -1;
    }
    int value = static_cast<int>(f + round);
    ...
    if (value < 0) {
        display.setSegments(MINUS, 1, 2 - length);
    }
}
...
void testLedSegments()
{
  ...
  for(k = 0; k < 7; k++) {
    ...
    Serial.println("Brightness=" + String(k));
  ...
  for(k = 3; k >= 0; k--) {
	display.setSegments(SEG_DEGREE, 1, k);
	delay(TEST_DELAY);
	}
